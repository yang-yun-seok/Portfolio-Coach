...
export class DataLoader {
  ...
  loadJobsSnapshot() {
    ...
    const filteredActiveJobs = filteredManagedJobs.filter(isActiveJob);
    ...
    const metadata = {
      ...
      activeJobsCount: filteredActiveJobs.length,
      inactiveJobsCount: Math.max(0, filteredManagedJobs.length - filteredActiveJobs.length),
  ...
  loadJobHistoryIndex(filters = {}) {
    ...
    return listJobHistoryFileNames(historyDir).map((fileName) => {
      ...
      return {
        ...
        referenceJobCount: filteredJobs.length || snapshot?.referenceJobCount || 0,
        newJobsCount: filteredJobs.length ? countNewJobsForDate(filteredJobs, date) : (snapshot?.newJobsCount || 0),
        activeJobsCount: filteredJobs.length || snapshot?.activeJobsCount || 0,
  ...
  ensureJobsLoaded() {
    if (this.isCacheStale() || !this.cache.allJobs || !this.cache.publicJobs || !this.cache.jobMetadata || !this.cache.jobHistoryIndex) {
      ...
      this.cache.jobHistoryIndex = this.loadJobHistoryIndex(snapshot.metadata.filters);
      this.cache.loadedAt = Date.now();
    }
  }
  ...
  getJobHistory(date) {
    ...
    const filters = this.getJobMetadata().filters || {};
    if (!Array.isArray(snapshot.jobs)) return snapshot;

    const filteredJobs = buildPublicJobs(snapshot.jobs, filters);
    return {
      ...snapshot,
      jobs: filteredJobs,
