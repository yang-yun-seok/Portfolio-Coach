...
async function extractJobHeader(page) {
  return page.evaluate(() => {
    ...
    const titleFromMeta = readMeta(['meta[property="og:title"]', 'meta[name="title"]']);
    const companyFromMeta = (titleFromMeta.match(/^\[([^\]]+)\]/) || [])[1] || '';

    const title = readText([
      '.view__header-title .corp-title h1',
      '.corp-title h1',
      '.recruit-view-tit h2',
      '.tplViewTitle h2',
      '.view-title h2',
      'h2.title',
    ]) || readValue(['#GI_Title']) || titleFromMeta.replace(/^\[[^\]]+\]\s*/, '');

    const company = readText([
      '.view__header-title .corp-name a',
      '.view__header-title .corp-name .txt',
      '.recruit-view-tit .corp-name a',
      '.recruit-view-tit .corp-name',
      '.tplViewTitle .corp-name a',
      '.tplViewTitle .corp-name',
      '.corp-name a',
      '.corp-name .txt',
      '.company-name',
    ]) || companyFromMeta;
...
const IMAGE_ONLY_DESCRIPTION = '이미지형 모집요강 (텍스트 추출 불가)';
...
function getCrawlerJobKey(id) {
  return String(id || '').replace(/^crawled-/, '');
}

function toCrawlerJobId(id) {
  const key = getCrawlerJobKey(id);
  return key ? `crawled-${key}` : '';
}
...
async function extractDescriptionFromContainer(handle) {
  const iframeTexts = [];
  let iframeHasImage = false;
  const iframes = await handle.$$('iframe');
  for (const iframe of iframes) {
    const frameObj = await iframe.contentFrame();
    if (!frameObj) continue;
    await frameObj.waitForSelector('body', { timeout: 5000 }).catch(() => {});
    const frameSnapshot = await frameObj.evaluate(() => {
      const body = document.body;
      return {
        text: body?.innerText?.trim() || '',
        hasImage: Boolean(body?.querySelector('img')),
      };
    }).catch(() => ({ text: '', hasImage: false }));
    if (frameSnapshot.text) iframeTexts.push(frameSnapshot.text);
    if (frameSnapshot.hasImage) iframeHasImage = true;
  }
  const iframeBody = iframeTexts.join('\n\n').trim();
  if (iframeBody.length >= 50) return iframeBody;

  const directSnapshot = await handle.evaluate((el, noiseSelectors) => {
    const clone = el.cloneNode(true);
    clone.querySelectorAll(noiseSelectors).forEach((node) => node.remove());
    return {
      text: clone.innerText?.trim() || '',
      hasImage: Boolean(clone.querySelector('img')),
    };
  }, NOISE_SELECTORS).catch(() => ({ text: '', hasImage: false }));
  if (directSnapshot.text.length >= 50) return directSnapshot.text;

  if (iframeHasImage || directSnapshot.hasImage) return IMAGE_ONLY_DESCRIPTION;
  return '';
}
...
async function extractJobDescription(page) {
  ...
  for (const selector of selectors) {
    ...
    if (text === IMAGE_ONLY_DESCRIPTION) return text;
    if (text.length >= 50) {
      return text;
    }
  }
  ...
  if (hasImageOnlyBody) return IMAGE_ONLY_DESCRIPTION;
...
export async function runCrawler({
  ...
  const existingJobsById = new Map(
    (existingJobs || []).filter(Boolean).map((job) => [getCrawlerJobKey(job.id), job]),
  );
  ...
  try {
    ...
    await page.waitForSelector('.detailSearch', { timeout: 30000 });
    await page.waitForSelector('.detailSearch input[type="checkbox"], .detailSearch input[type="radio"]', { timeout: 30000 });
    await page.waitForSelector('#dev-gi-list', { timeout: 30000 }).catch(() => {});
    ...
    const checkboxInfoList = await page.evaluate((tgts) => {
      ...
      const inputs = Array.from(document.querySelectorAll('.detailSearch input[type="checkbox"], .detailSearch input[type="radio"]'));
      const normalize = (value) => String(value || '').replace(/\(\d+\)/g, '').replace(/\s+/g, '');

      const findTargetInput = (target) => {
        ...
        return inputs.find((input) => {
          ...
          const dataName = normalize(input.dataset?.name);
          const inputValue = normalize(input.value);
          return labelText.includes(normalizedTarget)
            || dataName.includes(normalizedTarget)
            || inputValue === normalizedTarget;
        }) || null;
      };

      tgts.forEach(t => {
        const cb = findTargetInput(t);
        if (!cb) return;
        ...
        if (!results.some((item) => item.cbId === cb.id)) {
          results.push({ cbId: cb.id, tabIdx, tabBtnText, target: t });
        }
      });
      return results;
    }, targets);
    ...
    onProgress({
      stage: 'filter',
      message: `필터 매칭: ${checkboxInfoList.length}/${targets.length}`,
      percent: 6,
    });

    if (checkboxInfoList.length === 0) {
      const html = await page.content().catch(() => '');
      saveDebugHtml(debugDir, `missing-filter-controls-${Date.now()}.html`, html);
      throw new Error('게임잡 상세검색 필터를 찾지 못했습니다.');
    }
    ...
    listedJobIds = allJobs
      .map((job) => toCrawlerJobId(job.id))
      .filter(Boolean);

    if (allJobs.length === 0) {
      const html = await page.content().catch(() => '');
      saveDebugHtml(debugDir, `empty-list-${Date.now()}.html`, html);
    }

    onProgress({ stage: 'detail', message: `총 ${allJobs.length}개 공고 확보. 상세 수집 시작`, total: allJobs.length, current: 0, percent: 30 });

    // ═══ 3단계: 상세 페이지 수집 ═══
    const pendingJobCount = allJobs.filter((job) => !existingJobsById.has(String(job.id))).length;
    for (let i = 0; i < allJobs.length; i++) {
      if (signal?.aborted) throw new Error('중단됨');

      const job = allJobs[i];
      const existingJob = existingJobsById.get(String(job.id));
      ...
      try {
        if (existingJob) {
          normalizedJobs.push({
            ...
            company: existingJob.company || job.company,
            url: job.link || existingJob.url,
            experience: existingJob.experience || job.careerFromList || '',
          });
          reusedCount += 1;
          continue;
        }
    ...
    if (err.message === '중단됨') {
      ...
      return {
        ...
        fetchedCount: successCount,
        reusedCount,
        pendingJobCount: Math.max(0, listedJobIds.length - reusedCount - successCount),
        listedJobIds,
        errors: [...errors, '사용자에 의해 중단됨'],
        jobs: normalizedJobs,
      };
    }
    if (page) {
      const html = await page.content().catch(() => '');
      saveDebugHtml(debugDir, `crawler-error-${Date.now()}.html`, html);
    }
    throw err;
  } finally {
...
export async function crawlSingleJob({ giNo, dataDir }) {
  ...
  try {
    ...
    await new Promise(r => setTimeout(r, 2000));
    const header = await extractJobHeader(page);

    // 공고 제목 추출
    const title = header.title || await page.evaluate(() => {
      const el = document.querySelector('.recruit-view-tit h2, .tplViewTitle h2, .view-title h2, h2.title');
      return el ? el.innerText.trim() : '';
    });

    // 회사명 추출
    const company = header.company || await page.evaluate(() => {
