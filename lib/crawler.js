...
async function resolveBrowserRuntime() {
  ...
  if (systemExecutablePath) {
    ...
    return {
      ...
      label: systemExecutablePath.split(/[\\/]/).pop(),
    };
  }
  ...
  throw new Error(
    '크롤링에 사용할 Chromium 기반 브라우저를 찾을 수 없습니다. ' +
    'Chrome, Edge, Naver Whale 중 하나를 설치하거나, 배포 서버에서는 Puppeteer 번들 Chrome이 설치되도록 npm install을 다시 실행해주세요.'
  );
}
...
export async function runCrawler({
  ...
  const { puppeteer, executablePath, label } = await resolveBrowserRuntime();
  ...
  try {
    // ═══ 1단계: 브라우저 시작 & 필터 적용 ═══
    onProgress({ stage: 'init', message: `브라우저 시작 중... (${label})`, percent: 0 });
...
export async function crawlSingleJob({ giNo, dataDir }) {
  const { puppeteer, executablePath } = await resolveBrowserRuntime();
