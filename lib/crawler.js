 ...
 *   시스템에 설치된 Chrome / Edge를 자동으로 찾아서 사용.
 *   (Windows 10/11 기본 탑재 Edge 포함)
 */
...
export async function runCrawler({
  ...
  if (!executablePath) {
    throw new Error(
      ...
      'Google Chrome(https://www.google.com/chrome) 또는 ' +
      'Microsoft Edge를 설치한 후 다시 시도해주세요.'
    );
  }

  // ── puppeteer-core 동적 임포트 ────────────────────────────────────────
  let puppeteer;
  try {
    const mod = await import('puppeteer-core');
    puppeteer = mod.default ?? mod;
  } catch (e) {
    throw new Error('puppeteer-core가 설치되어 있지 않습니다. npm install 을 다시 실행해주세요.');
  }
  ...
  try {
    // ═══ 1단계: 브라우저 시작 & 필터 적용 ═══
    onProgress({ stage: 'init', message: `브라우저 시작 중... (${executablePath.split(/[\\/]/).pop()})`, percent: 0 });
...
/**
 * 단건 크롤링 — GI_No 하나로 GameJob 상세 페이지를 직접 크롤링 (puppeteer-core 전용)
 * @param {Object} options
...
export async function crawlSingleJob({ giNo, dataDir }) {
  ...
  if (!executablePath) {
    throw new Error(
      'Chrome 또는 Edge 브라우저를 찾을 수 없습니다. ' +
      'Google Chrome 또는 Microsoft Edge를 설치한 후 다시 시도해주세요.'
    );
  }
  ...
  try {
    ...
    throw new Error(
      'puppeteer-core가 설치되어 있지 않습니다. ' +
      '프로젝트 폴더에서 npm install 을 실행하거나, start.bat를 다시 실행해주세요.'
    );
  }
