...
- 현재 운영 구조를 크게 깨지 않되, 최종적으로 `Private GitHub + Replit + Firebase` 구조로 정리
- 비용을 가능한 한 낮게 유지하고, 무료 구간 운영을 우선 목표로 삼기
...
- Daily Crawl: 초기에는 `GitHub Actions` 유지, 이후 필요 시 이전 검토
...
즉, 지금 상태는 “도구형 웹앱”이고, 앞으로는  
`로그인 기반 운영형 서비스`이자 `Private GitHub + Replit 운영`을 목표로 하는 구조로 바뀌어야 한다.
...
이 설계의 최종 운영 원칙은 아래와 같다.

1. 사용자 입장에서는 무료로 접근 가능한 서비스여야 한다.
...
이 문서에서 말하는 “최종 아키텍처”는 Replit 운영 기준이다.

```mermaid
flowchart LR
  A["사용자 브라우저"] --> B["Replit 앱"]
  A --> C["Firebase Auth"]
...
- 현재 AI 서버: Render 유지
- 최종 운영 목표: Replit
- 남용 방지: 서버에서 Firebase 토큰 검증
- 회원가입: 공개하지 않음
- 계정 생성: 관리자 수동 생성
- 관리자 확인: 별도 관리자 화면
- 소스 저장소: GitHub Private
...
## 23. Replit 최종 운영 목표 및 이전 계획

### 23.1 전제

장기적으로 `GitHub Pages + Render` 대신 `Replit` 중심 운영으로 옮기는 것이 목표라면,  
이번 Firebase 인증/제출 구조는 그 전환을 막지 않게 설계해야 한다.

핵심 원칙:

1. 인증과 데이터 저장은 Firebase에 둔다.
2. 앱 서버는 현재 Render에서 시작하되, 나중에 Replit로 옮길 수 있게 한다.
...
장기적으로 Replit 운영은 충분히 가능하며, 이 문서의 최종 목표도 Replit 운영이다.  
다만 순서는 다음이 맞다.
...
이 프로젝트의 최종 기준은 아래 네 줄로 요약한다.

1. 저장소는 `GitHub Private`이어야 한다.
2. 사용자에게 제공되는 서비스는 장기적으로 `Replit`에서 운영한다.
3. 인증/DB/파일 저장은 `Firebase`를 사용한다.
4. 운영은 무료 구간을 우선 목표로 하되, 이를 유지하기 위해 가입 제한, 호출 제한, 제출 제한을 함께 둔다.
