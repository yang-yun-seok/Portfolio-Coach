    ...
    return (
      <div className="apple-module apple-module-intro p-8 animate-in fade-in">
        <div className="max-w-2xl mx-auto">
          <div className="mb-8">
            <h2 className="text-3xl font-bold text-slate-800 mb-2">과제 평가 연습</h2>
            <p className="text-slate-500">실제 게임회사 실무 과제 평가를 모의 체험합니다. 제한 시간 내 답안을 작성하고 피드백을 확인하세요.</p>
          </div>

          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-8 text-center">
              <h3 className="text-2xl font-black text-sky-400 tracking-tight mb-1">GAME DESIGN</h3>
              <p className="text-slate-400 text-sm font-medium">실무 역량 평가 테스트</p>
            </div>
            ...
            <div className="p-8 space-y-5">
              <div className="flex items-start gap-4 bg-red-50 p-4 rounded-xl border border-red-100">
                ...
                <div>
                  <h4 className="font-bold text-red-700 text-sm mb-1">보안 정책 안내 (모의)</h4>
                  <p className="text-xs text-red-600/80 leading-relaxed">
                    실제 평가와 동일한 환경에서 연습합니다. 화면 캡쳐/녹화 금지 규정 등 실전 분위기를 경험하세요.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4 bg-sky-50 p-4 rounded-xl border border-sky-100">
                ...
                <div>
                  ...
                  <p className="text-xs text-sky-600/80 leading-relaxed">
                    총 <strong>13문항</strong>, 제한 시간 <strong>55분</strong>. 서술형 + 코딩 + 이미지 분석 복합 문제입니다.
                  </p>
                </div>
              </div>
              ...
              <button
                onClick={handleStart}
                className="w-full py-4 mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition-all text-lg shadow-lg active:scale-[0.98] flex items-center justify-center gap-3"
              >
                <Play size={20} />
                평가 시작
              </button>
      ...
      <div className="apple-module apple-module-test flex flex-col h-full bg-slate-950 text-slate-100">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-4xl mx-auto py-8 px-6 space-y-10">
            {TEST_DATA.map((item) => (
              <div key={item.id} className="bg-slate-900/50 rounded-2xl p-8 border border-slate-800 transition-all hover:border-slate-700">
                <div className="flex justify-between items-start mb-5">
                  <div className="flex-1">
                    <h3 className="text-xl font-black mb-2 text-white tracking-tight">{item.question}</h3>
                    <p className="text-slate-400 text-sm opacity-80">{item.description}</p>
                  </div>
                  <span className="bg-slate-800 text-slate-500 px-3 py-1 rounded-full text-[10px] font-black uppercase ml-4 shrink-0">Q{item.id}</span>
                </div>
                  ...
                  <div className="mb-6 rounded-xl overflow-hidden border border-slate-700 bg-black relative group min-h-[300px] flex flex-col items-center justify-center">
                    {!imgError ? (
                      ...
                      <div className="flex flex-col items-center justify-center p-12 text-center space-y-4">
                        ...
                        <p className="text-slate-500 font-bold text-sm">이미지를 불러올 수 없습니다</p>
                        <p className="text-[10px] text-slate-600">File: {item.imageUrl}</p>
                        <button
                ...
                {/* 코드 예시 */}
                {item.example && (
                  <div className="mb-5 p-4 bg-black/40 rounded-xl border border-slate-800/50 font-mono text-xs text-emerald-400/90 leading-relaxed whitespace-pre-wrap">
                ...
                <div className="relative">
                  <textarea
                    ...
                    className={`w-full bg-slate-950/50 border border-slate-800 rounded-xl p-5 text-slate-200 placeholder:text-slate-700 focus:outline-none focus:border-sky-500/50 focus:ring-2 focus:ring-sky-500/10 transition-all resize-none min-h-[200px] ${
                      item.type === 'code' ? 'font-mono text-sm leading-relaxed' : 'text-base leading-relaxed'
                    }`}
                  />
                  <div className="absolute bottom-3 right-4 text-[10px] font-black text-slate-700 uppercase tracking-widest">
                    {(answers[item.id] || '').length}자
                  </div>
                </div>
              </div>
            ))}
            ...
            {TEST_DATA.map((item) => (
              <div key={item.id} className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
                <div className="bg-slate-50 px-8 py-5 border-b border-slate-200 flex justify-between items-center">
                  <h3 className="text-lg font-black text-slate-900 tracking-tight">{item.question}</h3>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Review {item.id}</span>
                </div>
                <div className="p-8 grid grid-cols-1 lg:grid-cols-5 gap-8">
                  ...
                  <div className="lg:col-span-3 space-y-3">
                    <h4 className="font-black text-slate-400 text-[10px] uppercase tracking-widest flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-sky-500"></span> 내 답안
                    </h4>
                    <div className={`p-6 rounded-xl border min-h-[120px] whitespace-pre-wrap text-sm leading-relaxed ${
                      item.type === 'code'
                        ? 'font-mono bg-slate-950 text-slate-200 border-slate-800'
                  ...
                  <div className="lg:col-span-2 space-y-4">
                    <div className="bg-sky-50 p-5 rounded-xl border border-sky-100">
                      <h4 className="font-black text-sky-800 text-[10px] mb-2 uppercase tracking-widest">출제 의도</h4>
                      <p className="text-sm text-sky-900/80 leading-relaxed">{item.feedback.intent}</p>
                    </div>
                    <div className="bg-emerald-50 p-5 rounded-xl border border-emerald-100">
                      <h4 className="font-black text-emerald-800 text-[10px] mb-2 uppercase tracking-widest">이상적인 답변</h4>
                      <p className="text-sm text-emerald-900/80 leading-relaxed">{item.feedback.ideal}</p>
                    </div>
                    <div className="bg-red-50 p-5 rounded-xl border border-red-100">
                      <h4 className="font-black text-red-800 text-[10px] mb-2 uppercase tracking-widest">피해야 할 답변</h4>
                      <p className="text-sm text-red-900/80 leading-relaxed">{item.feedback.avoid}</p>
                    </div>
                  </div>
                </div>
              </div>
